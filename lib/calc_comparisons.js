...
function calcRelative(expected, actual) {
    return Decimal(expected).minus(Decimal(actual));
}

function calcRelativeAbs(expected, actual) {
    return (Decimal(expected).minus(Decimal(actual))).abs();
}
